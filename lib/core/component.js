import { createElement, createTextNode, replaceNode } from './dom'
...
function markSlots(parts, placeholder) {
...
function collectSlots(template, placeholder) {
  ...
  while (0 !== stack.length) {
    ...
    if (node.content && '🦄' === node.content.firstChild.nodeValue) {
      replaceNode(createTextNode(''), node)
...
export function createComponent(parts) {
  ...
  if (!component) {
    const markup = markSlots(parts, placeholder)
