...
function pushPatch(type, newNode, oldNode) {
  patches.push(createPatch(type, newNode, oldNode))
}
...
// function prune(newNode, oldNode) {
//   for (let i = 0, length = newNode.children.length; i < length; i++) {
//     prune(newNode.children[i], oldNode.children[i])
//   }
...
function diffComponent(newNode, oldNode) {
  ...
  // // XXX
  // if (oldNode.bounds) {
  //   if (oldNode.bounds.top >= window.innerHeight || oldNode.bounds.bottom <= window.pageYOffset) {
  //     prune(newNode, oldNode)
  //     return
  //   }
  // }
